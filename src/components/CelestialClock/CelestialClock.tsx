...
type CelestialClockProps = {
  className?: string;
  /** Pause hand animations when the hero is off-screen */
  paused?: boolean;
};
...
export function CelestialClock({
  ...
  return (
    <div
      className={`${styles.clock} ${paused ? styles.paused : ""} ${className}`}
      aria-hidden="true"
    >
      <svg viewBox="0 0 240 240" fill="none" className={styles.svg}>
        <g stroke="currentColor" strokeLinecap="square" strokeLinejoin="miter">
          ...
          {/* sunburst ring - 12 rays keeps the look with less paint */}
